...
- [https://pirates.descodifi.cat/assoliments.html](https://pirates.descodifi.cat/assoliments.html)
    - **(La Sala del Tresor):** L'àlbum virtual on cada grumet pot veure els medallons i enganxines que ha desbloquejat superant els diferents reptes.
...
3. **Escriviu l'ID del repte:** A dalt de tot, poseu un identificador únic en minúscules i sense espais (ex: `repte_anells`). Si voleu modificar un repte existent, escriviu el seu ID i cliqueu a **"CARREGAR EXISTENT"**.
4. **Ompliu les dades generals:** * Títol, Temps Límite (en segons) i la Història inicial (podeu posar-hi etiquetes HTML bàsiques com `<b>` o `<br>`).
5. **Configureu l'Assoliment (Enganxina):**
...
8. **Repartiu el mapa:** Compartiu amb els alumnes l'enllaç de joc: `index.html?repte=repte_anells`.

---

# 🏴‍☠️ Base de Dades Pirates: Reptes SQL

A continuació es mostra la progressió dels reptes de la base de dades, ordenats per la dificultat de les clàusules i estructures SQL necessàries per resoldre'ls.
